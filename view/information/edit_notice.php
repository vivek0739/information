...
<?php
	...
	$form = $ui->form()->extras('enctype="multipart/form-data"')
		->action('information/edit_notice/edit/'.$notice_row->notice_id.'/'.$auth_id)->open();
	...
	$inputRow3 = $ui->row()->open();
		...
		echo '<br/><a href="'.base_url().'assets/files/information/notice/'.$notice_row->notice_path.'" title="download file" download="'.$notice_row->notice_path.'">'.$notice_row->notice_path.'</a>';
	    $coll->close();
 ...
 $inputRow4 = $ui->row()->open();
	$innercol8 = $ui->col()->id('category_col')->width(6)->open();
		$ui->select()
		   ...
		   ->name('cat_selection')
		   ->id("cat_selection")
		   ->options(array($ui->option()->value('all')->text('All'),
...
$(document).ready(function() {
	...
	document.getElementById('notice_cat').value='<? echo $notice_row->notice_cat?>';
	var notice_cat=document.getElementById('notice_cat').value;
	if(notice_cat=='ind')
	{
		...
		$(window).load(function()
		{
			...
			index_individual(people,'<? echo $notice_row->notice_id?>')
		});
	...
	else if(notice_cat=='gen')
	{
		$(window).load(function()
		{
			...
			$('#department_col').show();
			general_col();
			document.getElementById('cat_selection').value='<? if($d_general!=1) echo $d_general[0]->notice_cat;?>';
			document.getElementById('dept_selection').value='<? if($d_general!=1) echo $d_general[0]->dept_id;?>';
			...
			if(notice_cat_sub=='stu')
			{
				...
				var course='<? if($d_general_stu!=1) echo $d_general_stu[0]->course_id;?>';
