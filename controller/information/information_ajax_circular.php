...
class Information_ajax_circular extends MY_Controller
{
	...
	public function index_individual($no_of_individuals,$circular_id)
	{
		$this->load->model('information/view_circular_model');
		$data['individuals']=$this->view_circular_model
										->get_individual($circular_id)->result();
	...
	{
		$query=$this->post_circular_help_model->create_group();
		$sno=1;
	...
	public function add_member($data1,$data2,$data3,$data4)
	{
		...
		else
		{
			...
			$this->post_circular_help_model->put_group_class($data1,$data2,$data3,$data4);
			$data['members']=$this->post_circular_help_model->get_group_member($data1,$data2);
			$data['data1']=$data1;
			$data['data2']=$data2;
			$data['data4']=$data4;
	...
	public function rm_member($data1,$data2,$data3,$data4)
	{
		...
		$this->post_circular_help_model->del_group_class($data1,$data2,$data3,$data4);
		$data['members']=$this->post_circular_help_model->get_group_member($data1,$data2);
		$data['data1']=$data1;
		$data['data2']=$data2;
		$data['data4']=$data4;
		if($data['members']==NULL)
		{
			...
			$this->post_circular_help_model->del_group_notice($data1,$data2);
		}
		$this->load->view('information/group/group_member',$data);
		$this->drawFooter ();
	}
	public function semester($course,$sem='')
	{
		...
		$data['course']=$this->post_circular_help_model->get_course_where($course);
		$data['sem']=$sem;
	...
	public function get_courses($dept,$courses='')
	{
		...
		if($dept=='all')
		{
			$data['course']=$this->post_circular_help_model->get_course();
	...
	{
		$this->post_circular_help_model->del_group($group_name);
	}
