...
class Information_ajax_minutes extends MY_Controller
{
	...
	public function index_individual($no_of_individuals,$minute_id)
	{
		$this->load->model('information/view_minute_model');
		$data['individuals']=$this->view_minute_model
										->get_individual($minute_id)->result();
	...
	{
		$query=$this->post_minute_help_model->create_group();
		$sno=1;
	...
	public function add_member($data1,$data2,$data3,$data4)
	{
		...
		else
		{
			...
			$this->post_minute_help_model->put_group_class($data1,$data2,$data3,$data4);
			$data['members']=$this->post_minute_help_model->get_group_member($data1,$data2);
			$data['data1']=$data1;
			$data['data2']=$data2;
			$data['data4']=$data4;
	...
	{
		$result=$this->post_minute_help_model->search_group($data1);
		if($result==1)
	}*/
	public function rm_member($data1,$data2,$data3,$data4)
	{

		$this->post_minute_help_model->del_group_class($data1,$data2,$data3,$data4);
		$data['members']=$this->post_minute_help_model->get_group_member($data1,$data2);
		$data['data1']=$data1;
		$data['data2']=$data2;
		$data['data4']=$data4;
		if($data['members']==NULL)
		{
			...
			$this->post_minute_help_model->del_group_notice($data1,$data2);
		}
		$this->load->view('information/group/group_member',$data);
		$this->drawFooter ();
	}
	public function semester($course,$sem='')
	{
		...
		$data['course']=$this->post_minute_help_model->get_course_where($course);
		$data['sem']=$sem;
	...
	public function get_courses($dept,$courses='')
	{
		...
		if($dept=='all')
		{
			$data['course']=$this->post_minute_help_model->get_course();
	...
	{
		$this->post_minute_help_model->del_group($group_name);
	}
